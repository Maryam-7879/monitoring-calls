...
import { Survey, SurveyQuestion, Queue } from '../../../types/surveyManagement';
...
const SurveyForm: React.FC<SurveyFormProps> = ({
  ...
  useEffect(() => {
    if (survey) {
      setName(survey.name);
      setSelectedQueue(survey.queue);
      setCustomDestination(survey.customDestination || '');
  ...
  const validate = (): boolean => {
    ...
    if (!selectedQueue) {
      newErrors.queue = 'انتخاب صف الزامی است';
    }
  ...
  const handleApplyDialplan = async () => {
    if (!validate()) {
      toast.error('لطفاً ابتدا نظرسنجی را ذخیره کنید');
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">تنظیمات و سؤالات نظرسنجی را وارد کنید</p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="مثال: نظرسنجی واحد فروش"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              انتخاب صف <span className="text-red-500">*</span>
            </label>
            <select
              value={selectedQueue}
              onChange={(e) => setSelectedQueue(e.target.value)}
              onFocus={onFetchQueues}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                errors.queue ? 'border-red-500' : 'border-slate-300'
              }`}
              disabled={loading}
            >
              <option value="">یک صف انتخاب کنید</option>
              {queues.map((queue) => (
                <option key={queue.id} value={queue.number}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="مثال: ext-local,200,1"
